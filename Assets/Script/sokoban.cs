...
public class sokoban : MonoBehaviour
{
        ...
        {
                // タイルの情報を一行ごとに分割
                var lines = stageFile.text.Split//txtファイル読み込み
                            (
                        new[] { '\r', '\n' },//改行で区切る
                        System.StringSplitOptions.RemoveEmptyEntries//空白を含めないオプション
                            );

                // タイルの列数を計算
                var nums = lines[ 0 ].Split( new[] { ',' } );
                //Debug.Log(nums[0]+":"+lines[0]);
                // タイルの列数と行数を保持
                ...
                //Debug.Log(tileList[ 1, 1 ]);
        }
        public float tileSize;  // タイルのサイズ
        public Sprite keepoutblock; // 立ち入り禁止のスプライト
        ...
        public Vector2 middleOffset; // 中心位置
...
// 各位置に存在するゲームオブジェクトを管理する連想配列
                ...
                for ( int y = 0; y < rows; y++ )
                {
                        for ( int x = 0; x < columns; x++ )
                        {
                                ...
                                if ( val == TileType.NONE ) {
                                        var keepout = new GameObject( "keepout" );
                                        sr = keepout.AddComponent<SpriteRenderer>();
                                        sr.sprite =  keepoutblock;
                                        sr.sortingOrder = 1;
                                        keepout.transform.position = GetDisplayPosition( x, y );
                                        //continue;
                                }
                                // 目的地の場合
                                ...
                                if ( val == TileType.PLAYER )
                                {
                                        ...
                                        collider.size=new Vector2(tileSize/2,tileSize/2);
        ...
        private Vector2Int GetNextPositionAlong( Vector2Int pos, DirectionType direction )
        {
                ...
                case DirectionType.DOWN:
                        pos.y += 1;
                        if(pos.y>=columns) pos.y=0;
                        break;

                // 左
                case DirectionType.LEFT:
                        pos.x -= 1;
                        if(pos.x<0) pos.x=rows-1;
                        break;
                }
